...
export function DinoGame() {
  ...
  const jump = () => {
    if (!isGameOver && gameStarted && dinoY <= 1 && !isJumping) {
      setIsJumping(true)
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      obstaclesRef.current.forEach(obs => {
        ...
        const dinoTop = 120 - DINO_HEIGHT/2 - dinoYRef.current
        const dinoBottom = dinoTop + DINO_HEIGHT

        const obsLeft = obs.x
        const obsRight = obs.x + obs.width
        const obsTop = 120 - obs.height/2
  ...
  return (
    <div className="relative w-full bg-transparent rounded-lg overflow-hidden">
      {/* Game Canvas */}
      <div 
        className="relative w-full h-[240px] cursor-pointer select-none"
      ...
      >
        ...
        {gameStarted && (
          <motion.div
            ...
            style={{
              left: '50px',
              top: `${120 - DINO_HEIGHT/2 - dinoY}px`,
              width: `${DINO_WIDTH}px`,
